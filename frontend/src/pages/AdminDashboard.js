...
import { Users, FileText, CreditCard, Globe, Image, Video, Settings, Clock, Activity, DollarSign } from 'lucide-react';
...
const AdminDashboard = ({ user }) => {
  ...
  const quickActions = [
    { title: 'إدارة الأسعار', desc: 'تحكم بالباقات والعروض والخصومات', path: '/admin/pricing', icon: <DollarSign className="w-6 h-6" />, color: 'from-green-500 to-emerald-500' },
    ...
    { title: 'إدارة المدفوعات', desc: 'مراجعة والموافقة على المدفوعات', path: '/admin/payments', icon: <CreditCard className="w-6 h-6" />, color: 'from-yellow-500 to-orange-500' },
              ...
              {statCards.map((stat, idx) => (
                <Card key={idx} className="bg-slate-800 border-slate-700">
                  <CardContent className="p-4 text-center">
                    ...
                    <span className="text-2xl font-bold text-white block">{stat.value || 0}</span>
